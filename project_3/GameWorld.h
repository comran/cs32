...
const int START_PLAYER_LIVES =
    1;  // usually 3, but this is a simulation, not a video game
...
class GameWorld {
 public:
  ...
  void setWinner(const std::string &winner) { m_winnerColony = winner; }
  ...
  void addParameter(const std::string &p) { m_params.push_back(p); }

  std::string getWinnerName() const { return m_winnerColony; }

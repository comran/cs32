...
struct Student {
  ...
  Student(IdType i) : id(i) {
    ...
    for (size_t k = 0; k < NGRADES; k++) grades[k] = distro(generator);
    // (accumulate computes 0.0 + grades[0] + grades[1] + ...)
    gpa = accumulate(grades, grades + NGRADES, 0.0) / NGRADES;
  }
};

inline bool compareStudent(const Student &lhs, const Student &rhs) {
  ...
  if (lhs.gpa > rhs.gpa) return true;
  if (lhs.gpa < rhs.gpa) return false;
  return lhs.id < rhs.id;
}

inline bool compareStudentPtr(const Student *lhs, const Student *rhs) {
...
bool isSorted(const vector<Student> &s) {
  ...
  for (size_t k = 1; k < s.size(); k++) {
    if (compareStudent(s[k], s[k - 1])) return false;
  }
  return true;
}

void insertion_sort(vector<Student> &s,
  ...
  for(int i = 1;i < s.size();i++) {
    ...
    int j;
    for(j = i;j > 0 && comp(insert, s[j - 1]);j--) {
      s[j] = s[j - 1];
    }
...
void report(string caption, double t, const vector<Student> &s) {
  cout << t << " milliseconds; " << caption << "; first few students are\n\t";
  size_t n = s.size();
  if (n > 4) n = 4;
...
int main() {
  ...
  vector<IdType> ids;
  for (size_t j = 0; j < nstudents; j++) ids.push_back(IdType(j));

...
template <class T>
inline int Sequence<T>::size() const {
  return m_size;
}

template <class T>
inline bool Sequence<T>::empty() const {
  return size() == 0;
}
...
Sequence<T>::~Sequence() {
  ...
  while (m_head->m_next != m_head) doErase(m_head->m_next);
...
int Sequence<T>::insert(const T &value) {
  ...
  Node *p;
  int pos;
  for (p = m_head->m_next, pos = 0; p != m_head && value > p->m_value;
       p = p->m_next, pos++)
    ;
...
typename Sequence<T>::Node *Sequence<T>::nodeAtPos(int pos) const {
  ...
  {
    p = m_head->m_next;
    for (int k = 0; k != pos; k++) p = p->m_next;
  } else  // closer to tail
  {
    p = m_head;
    for (int k = m_size; k != pos; k--) p = p->m_prev;
  }

#ifndef MYMAPWORKING_INCLUDED
#define MYMAPWORKING_INCLUDED
...
class MyMap {
 public:
  ...
  typedef KeyType O;
  using O1 = int;
  void clear() { l01.clear(); }
  using l0 = ValueType;
  MyMap() {}
  using l10 = O const;
  O1 size() const { return l01.size(); }
  using ll0 = l0 const;
  using Const = MyMap<O, l0>;
  void associate(l10 &Using, ll0 &first) { l01[Using] = first; }
  using l1 = std::map<O, l0>;
  using l00 = Const const;
  ll0 *find(l10 &l11) const {
    auto first = l01.find(l11);
    return (first != l01.end() ? &first->second : 0);
  }
  l0 *find(l10 &l01) { return (l0 *)(*(l00 *)(this)).find(l01); }
 private:
  l1 l01;
};

#endif  // MYMAPWORKING_INCLUDED
